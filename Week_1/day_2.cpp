...
vector<int> update_pos(string command, int magnitude, vector<int> pos)
{
...
int main()
{
    std::vector<int> my_pos = {0, 0}; // 2D array in form [depth, x position] to define current position
    ...
    while (getline (MyReadFile, line)) {
        ...
        my_pos = update_pos(tokens[0], stoi(tokens[1]), my_pos);
    }
